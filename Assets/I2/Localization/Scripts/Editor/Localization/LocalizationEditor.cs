...
namespace I2.Loc
{
	public partial class LocalizationEditor
	{
		...
		public enum eViewMode { ImportExport, Keys, Languages, Tools, References, Settings };
		...
		private int localizationFormatBehaviourTypesSelected = -1;
		...
		void OnGUI_Main()
		{
			...
			GUILayout.BeginHorizontal();
				...
				OnGUI_ToggleEnumBig("Settings", ref mCurrentViewMode, eViewMode.Settings, null, null);
			...
			switch (mCurrentViewMode)
			{
				...
				case eViewMode.Settings					: OnGUI_Settings(); break;
			}
		}
		...
		private void OnGUI_Settings()
		{
			GUILayout.BeginVertical(EditorStyles.helpBox);
			{
				...
				string[] types = sourceData.LocalizationFormatBehaviourTypes;
				string[] fontTerms = sourceData.FontTerms;
				string[] languages = LocalizationManager.GetAllLanguages().ToArray();

				InitPopup(types, x => x == sourceData.localizationFormatBehaviourType, ref localizationFormatBehaviourTypesSelected);
				...
				GUILayout.BeginVertical(EditorStyles.helpBox);
				{
					...
					localizationFormatBehaviourTypesSelected = EditorGUILayout.Popup("Localization Format Behaviour Type", 
						localizationFormatBehaviourTypesSelected, types);
				...
				GUILayout.BeginVertical(EditorStyles.helpBox);
				{
					...
					GUILayout.BeginHorizontal();
					{
						GUILayout.Label("Use Invariant Culture As Default");
						sourceData.useInvariantCultureAsDefault = GUILayout.Toggle(sourceData.useInvariantCultureAsDefault, string.Empty);
					}
					GUILayout.EndHorizontal();
				...
				TrySetValueByIndex(types, ref localizationFormatBehaviourTypesSelected, ref sourceData.localizationFormatBehaviourType);
			}
			GUILayout.EndVertical();

			void InitPopup<T>(T[] values, Predicate<T> match, ref int index)
			{
				if (index >= 0)
				{
					return;
				...
				int foundIndex = Array.FindIndex(values, match);

				index = foundIndex >= 0 ? foundIndex : 0;
			}

			void TrySetValueByIndex<T>(T[] values, ref int index, ref T value)
			{
		...
		void OnGUI_References()
		{
            ...
            var testAddObj = (canTest && LocalizationEditor.mTestAction == LocalizationEditor.eTest_ActionType.Button_Assets_Add) ? (UnityEngine.Object)LocalizationEditor.mTestActionArg : null;
            ...
            var testReplaceObj = (canTest && LocalizationEditor.mTestAction == LocalizationEditor.eTest_ActionType.Button_Assets_Replace) ? (UnityEngine.Object)LocalizationEditor.mTestActionArg2 : null;
